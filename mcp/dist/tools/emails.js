...
export function registerEmailTools(server, supabase) {
    // ── get_thread ──────────────────────────────────────────────────────────────
    server.tool('get_thread', 'Get the email thread for a case channel. Returns messages with PII and full body redacted — only subject, preview, direction, and timestamps are returned.', {
        type: 'object',
        required: ['case_id'],
        properties: {
        ...
        return {
            messages: applyPrivacyList('email_messages', (data || [])),
            count: data?.length ?? 0,
            _privacy: 'sender_email, recipient_email, cc, and body_text redacted. Use body_preview for AI context.',
        };
    });
    // ── get_thread_summary ──────────────────────────────────────────────────────
    server.tool('get_thread_summary', 'Get the AI-generated rolling summary for a case thread. Includes tone, open questions, risks, and promises made.', {
        type: 'object',
        required: ['case_id'],
        properties: {
            case_id: { type: 'string', description: 'The case UUID' },
        ...
        let query = supabase
            ...
            .eq('case_id', args.case_id);
        if (args.channel_type)
            query = query.eq('channel_type', args.channel_type);
        ...
        if (!data)
            return { summary: null };
        return {
            summary: applyPrivacyList('thread_summaries', [data])[0],
        };
    });
}
