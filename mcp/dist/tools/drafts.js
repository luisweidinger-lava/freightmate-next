export function registerDraftTools(server, supabase, env) {
    // ── list_pending_drafts ─────────────────────────────────────────────────────
    server.tool('list_pending_drafts', 'List AI-generated message drafts that are awaiting approval. Optionally filter by case.', {
        type: 'object',
        properties: {
    ...
    // ── request_draft ───────────────────────────────────────────────────────────
    server.tool('request_draft', 'Trigger AI draft generation for a case channel (calls n8n WF5).', {
        type: 'object',
    ...
    // ── approve_draft ───────────────────────────────────────────────────────────
    server.tool('approve_draft', 'Approve an AI draft for sending. Calls the approve-draft webhook which triggers WF6.', {
        type: 'object',
        required: ['draft_id'],
        properties: {
