...
export function registerCaseTools(
  ...
  server.tool(
    'list_cases',
    'List active shipment cases. Optionally filter by status or priority. Returns sanitized case data.',
    {
      type: 'object',
      properties: {
    ...
    async (args) => {
      ...
      return {
        cases: applyPrivacyList('shipment_cases', (data || []) as Record<string, unknown>[]),
        count: data?.length ?? 0,
  ...
  server.tool(
    'get_case',
    'Get full detail for a single shipment case by its case ID or ref_number.',
    {
      type: 'object',
      required: ['id_or_ref'],
      properties: {
    ...
    async (args) => {
      const val = args.id_or_ref as string
      ...
      if (!caseData) throw new Error(`Case not found: ${val}`)

      return {
        case: applyPrivacy('shipment_cases', caseData as Record<string, unknown>),
        _privacy: 'client_email and client_name redacted per privacy policy',
      }
    },
  )
  ...
  server.tool(
    'get_case_channels',
    'Get communication channels (client and vendor) for a case.',
    {
      type: 'object',
      required: ['case_id'],
      properties: {
