...
export async function POST(req: Request) {
  ...
  // Fetch grant and verify caller is the operator
  const { data: grant } = await admin.from('case_access_grants').select('*').eq('id', grant_id).single()
  if (!grant) return NextResponse.json({ error: 'Grant not found' }, { status: 404 })
  if (grant.operator_id !== user.id) return NextResponse.json({ error: 'Forbidden' }, { status: 403 })

  const update: Record<string, unknown> = { status: 'granted', resolved_at: new Date().toISOString() }
