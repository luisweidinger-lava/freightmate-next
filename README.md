# Nexio - The Nexus of Freight Forwarding Logistics
> An AI-powered coordination layer for freight forwarding SMEs — built to enhance existing workflows, not replace them.
...
The platform handles the full coordination lifecycle: job assignment, contractor management, shipment tracking, and client communication — all structured, tracked, and summarised automatically.
...
- **Unmanageable email volume** — coordinators process a high volume of emails daily, all sorted chronologically and mixed across cases, clients, and contractors, with no structure to distinguish between them at a glance
- **Poor document retrieval** — files and documents shared via email are difficult to locate, as coordinators must remember exactly which email a document came from with no centralised storage per case
- **Knowledge locked to one coordinator** — each case relies entirely on the assigned coordinator's personal knowledge, leaving no flexibility to reassign it to another operator
- **Missed follow-ups** — without a structured tracking system, emails can go unanswered for extended periods, leading to undetected delays in the coordination chain
- **No managerial visibility** — managers have no practical way to understand the status of ongoing cases. Without a structured overview, gaining any meaningful insight into workload distribution, case progress, or team activity is simply not feasible
- **Absence of in-context collaboration** — there is no way for managers to engage with coordinators directly on a specific case. Communication relies on in-person conversations, disconnected from the actual work
---
 
## The Market
 
Kenya is East Africa's dominant logistics hub, with the sector valued at approximately **USD 1.4 billion (2023)** and projected to reach **USD 10.3 billion by 2032** (CAGR ~24.8%). Road freight is the primary mode, serving both domestic routes and landlocked neighbouring countries including Uganda, Rwanda, and South Sudan. Kenya's freight and warehousing sector comprises over **1,200 registered companies** under KIFWA (Kenya International Freight and Warehousing Association), the majority of which are small and mid-sized operators — representing Nexio's core addressable market. The actual number of active logistics operators in the country is likely higher, as a portion of the market operates outside formal registration.
...
| Database          | **Supabase** (PostgreSQL + PostgREST) — source of truth for all app state           |
| Authentication    | **Supabase Auth** — role-based access control separating coordinator and manager permissions |
